...
from backend.ahmedabadBackend import AhmedabadMainElements
...
layout = html.Div(id='ahmedabadParent', children=[

    # html.Header(id='header', children=[
    #     html.H1("ahmedabad")
    #     # html.Img(id='displayImage',src=app.get_asset_url(f"{rootDirectory}/Air-Quality-Index-Prediction/photos/ahmedabad.jpg"))
    # ]),
    headerComponent(cityName, "November 2017", math.floor(cityAQI[cityName])),
        ...
        dbc.Card(
            dbc.CardBody(id='card', children=[
                ...
                dbc.Row(className='cardBody', children=[
                    cardLayout(html.Div(
                        dcc.Graph(id='ahmedabadGasesLinedGraph', className='graphPlot', figure={})))
                ...
                dbc.Row(children=[
                    ...
                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(
                            dcc.Graph(id='ahmedabadGasesBoxPlot', className='graphPlot', figure={})))
                    ], width=7),

                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(
                            dcc.Graph(id='ahmedabadGasesMonthlyPlot', className='graphPlot', figure={})))
                    ], width=5)
                ]),
                html.Br(), html.Br(),
#html.Img(src=app.get_asset_url("../assets/photos/ahmedabadImage.jpg"))
                dbc.Row(children=[
                    cardLayout(html.Img(src=app.get_asset_url("/photos/ahmedabadImage.jpg"), style={
                        'height': '500px',
                        'width': '1450px',
                    })),
                    html.Br(), html.Br(),
                ...
                dbc.Row(children=[
                    dbc.Collapse(id='ahm_collapse', is_open=False, children=[
                        ...
                        cardLayout(
                            html.Iframe(id="ahm_comp_analysis", srcDoc="", style={
                                'height': '500px',
                                'width': '1450px',
                            })
                        ),
                ...
                dbc.Row(children=[
                    cardLayout(html.Iframe(srcDoc=AhmedabadMainElements.comparingScenarios(), style={
                        'height': '500px',
                        'width': '1450px',
                    })),
                    html.Br(), html.Br(),
...
@app.callback(
    Output(component_id='ahm_comp_analysis', component_property='srcDoc'),
    Input(component_id='slct_metric', component_property='value')
)
def comparitiveAnalysis(value):
    if value == "rmse":
        srcDoc = AhmedabadMainElements.comparativeAnalysisRMSE()
    elif value == "mape":
        srcDoc = AhmedabadMainElements.comparativeAnalysisMAPE()
    elif value == "mae":
        srcDoc = AhmedabadMainElements.comparativeAnalysisMAE()
    elif value == "me":
        srcDoc = AhmedabadMainElements.comparativeAnalysisME()
    elif value == "mse":
        srcDoc = AhmedabadMainElements.comparativeAnalysisMSE()
    elif value == "mpe":
        srcDoc = AhmedabadMainElements.comparativeAnalysisMPE()
    else:
        srcDoc = None
...
@app.callback(
    [Output(component_id='ahmedabadGasesLinedGraph', component_property='figure'),
     Output(component_id='ahmedabadGasesBoxPlot', component_property='figure'),
     Output(component_id='ahmedabadGasesMonthlyPlot',
...
def dropdownGraphs(slct_gas):
    ...
    monthlyData = city.groupby("month", sort=False)[
        'PM2.5', 'NO2', 'NO', 'NOx', 'CO', 'SO2', 'O3', 'AQI'].mean().reset_index()
...
@app.callback(
    Output("ahm_collapse", "is_open"),
    [Input("ahm_collapse-button", "n_clicks")],
    [State("ahm_collapse", "is_open")],
)
def toggle_ahm_collapse(n, is_open):
    if n:
        return not is_open
    return is_open
