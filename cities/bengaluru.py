...
from backend.bengaluruBackend import BengaluruMainElements
...
layout = html.Div(id='bengaluruParent', children=[

    # html.Header(id='header', children=[
    #     html.H1("bengaluru")
    #     # html.Img(id='displayImage',src=app.get_asset_url(f"{rootDirectory}/Air-Quality-Index-Prediction/photos/bengaluru.jpg"))
    # ]),
        ...
        dbc.Card(
            dbc.CardBody(id='card', children=[
                ...
                dbc.Row(className='cardBody', children=[
                    cardLayout(html.Div(
                        dcc.Graph(id='bengaluruGasesLinedGraph', className='graphPlot', figure={})))
                ...
                dbc.Row(children=[
                    ...
                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(
                            dcc.Graph(id='bengaluruGasesBoxPlot', className='graphPlot', figure={})))
                    ], width=7),

                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(
                            dcc.Graph(id='bengaluruGasesMonthlyPlot', className='graphPlot', figure={})))
                    ], width=5)
                ]),
                html.Br(), html.Br(),

                dbc.Row(children=[
                    cardLayout(html.Iframe(srcDoc=BengaluruMainElements.html_arima(), style={
                        'height': '500px',
                        'width': '1450px',
                    })),
                    html.Br(), html.Br(),
                ...
                dbc.Row(children=[
                    dbc.Collapse(id='ben_collapse', is_open=False, children=[
                        ...
                        cardLayout(
                            html.Iframe(id="ben_comp_analysis", srcDoc="", style={
                                'height': '500px',
                                'width': '1450px',
                            })
                        ),
                ...
                dbc.Row(children=[
                    cardLayout(html.Iframe(srcDoc=BengaluruMainElements.comparingScenarios(), style={
                        'height': '500px',
                        'width': '1450px',
                    })),
                    html.Br(), html.Br(),
...
@app.callback(
    Output(component_id='ben_comp_analysis', component_property='srcDoc'),
    Input(component_id='slct_metric', component_property='value')
)
def comparitiveAnalysis(value):
    if value == "rmse":
        srcDoc = BengaluruMainElements.comparativeAnalysisRMSE()
    elif value == "mape":
        srcDoc = BengaluruMainElements.comparativeAnalysisMAPE()
    elif value == "mae":
        srcDoc = BengaluruMainElements.comparativeAnalysisMAE()
    elif value == "me":
        srcDoc = BengaluruMainElements.comparativeAnalysisME()
    elif value == "mse":
        srcDoc = BengaluruMainElements.comparativeAnalysisMSE()
    elif value == "mpe":
        srcDoc = BengaluruMainElements.comparativeAnalysisMPE()
    else:
        srcDoc = None
...
@app.callback(
    [Output(component_id='bengaluruGasesLinedGraph', component_property='figure'),
     Output(component_id='bengaluruGasesBoxPlot', component_property='figure'),
     Output(component_id='bengaluruGasesMonthlyPlot',
...
@app.callback(
    Output("ben_collapse", "is_open"),
    [Input("ben_collapse-button", "n_clicks")],
    [State("ben_collapse", "is_open")],
)
def toggle_ben_collapse(n, is_open):
    if n:
        return not is_open
    return is_open
