...
from backend.gandhinagarBackend import GandhinagarMainElements
...
layout = html.Div(id='gandhinagarParent', children=[

    # html.Header(id='header', children=[
    #     html.H1("gandhinagar")
    #     # html.Img(id='displayImage',src=app.get_asset_url(f"{rootDirectory}/Air-Quality-Index-Prediction/photos/gandhinagar.jpg"))
    # ]),
        ...
        dbc.Card(
            dbc.CardBody(id='card', children=[
                ...
                dbc.Row(className='cardBody', children=[
                    cardLayout(html.Div(
                        dcc.Graph(id='gandhinagarGasesLinedGraph', className='graphPlot', figure={})))
                ...
                dbc.Row(children=[
                    ...
                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(
                            dcc.Graph(id='gandhinagarGasesBoxPlot', className='graphPlot', figure={})))
                    ], width=7),

                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(
                            dcc.Graph(id='gandhinagarGasesMonthlyPlot', className='graphPlot', figure={})))
                    ], width=5)
                ]),
                html.Br(), html.Br(),

                dbc.Row(children=[
                    cardLayout(html.Iframe(srcDoc=GandhinagarMainElements.html_arima(), style={
                        'height': '500px',
                        'width': '1450px',
                    })),
                    html.Br(), html.Br(),
                ...
                dbc.Row(children=[
                    dbc.Collapse(id='gan_collapse', is_open=False, children=[
                        ...
                        cardLayout(
                            html.Iframe(id="gan_comp_analysis", srcDoc="", style={
                                'height': '500px',
                                'width': '1450px',
                            })
                        ),
                ...
                dbc.Row(children=[
                    cardLayout(html.Iframe(srcDoc=GandhinagarMainElements.comparingScenarios(), style={
                        'height': '500px',
                        'width': '1450px',
                    })),
                    html.Br(), html.Br(),
...
@app.callback(
    Output(component_id='gan_comp_analysis', component_property='srcDoc'),
    Input(component_id='slct_metric', component_property='value')
)
def comparitiveAnalysis(value):
    if value == "rmse":
        srcDoc = GandhinagarMainElements.comparativeAnalysisRMSE()
    elif value == "mape":
        srcDoc = GandhinagarMainElements.comparativeAnalysisMAPE()
    elif value == "mae":
        srcDoc = GandhinagarMainElements.comparativeAnalysisMAE()
    elif value == "me":
        srcDoc = GandhinagarMainElements.comparativeAnalysisME()
    elif value == "mse":
        srcDoc = GandhinagarMainElements.comparativeAnalysisMSE()
    elif value == "mpe":
        srcDoc = GandhinagarMainElements.comparativeAnalysisMPE()
    else:
        srcDoc = None
...
@app.callback(
    [Output(component_id='gandhinagarGasesLinedGraph', component_property='figure'),
     Output(component_id='gandhinagarGasesBoxPlot', component_property='figure'),
     Output(component_id='gandhinagarGasesMonthlyPlot',
...
@app.callback(
    Output("gan_collapse", "is_open"),
    [Input("gan_collapse-button", "n_clicks")],
    [State("gan_collapse", "is_open")],
)
def toggle_gan_collapse(n, is_open):
    if n:
        return not is_open
    return is_open
