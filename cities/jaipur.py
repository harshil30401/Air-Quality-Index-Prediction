...
layout = html.Div(id = 'parent', children = [
    ...
    html.Header(id='header', children=[
        html.Img(src=app.get_asset_url(f"{rootDirectory}/Air-Quality-Index-Prediction/photos/jaipur.jpg"))
    ]),
        ...
        dbc.Card(
            dbc.CardBody(id= 'card', children=[
                ...
                dbc.Row(className='cardBody', children=[
                    cardLayout(html.Div(dcc.Graph(id = 'jaipurGasesLinedGraph', className='graphPlot', figure = {})))
                ], style={'padding':'5px', 'color':'blue'}),

                dbc.Row(children=[

                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(dcc.Graph(id = 'jaipurGasesBoxPlot', className='graphPlot', figure = {})))
                    ], width=7),

                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(dcc.Graph(id = 'jaipurGasesMonthlyPlot', className='graphPlot', figure = {})))
                    ], width=5)
                ]),
...
@app.callback(
    ...
    Output(component_id='jaipurGasesMonthlyPlot', component_property='figure')
    ],
