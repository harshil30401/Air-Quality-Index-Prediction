...
from backend.jodhpurBackend import JodhpurMainElements
...
layout = html.Div(id='jodhpurParent', children=[

    # html.Header(id='header', children=[
    #     html.H1("Jodhpur")
    #     # html.Img(id='displayImage',src=app.get_asset_url(f"{rootDirectory}/Air-Quality-Index-Prediction/photos/jodhpur.jpg"))
    # ]),
        ...
        dbc.Card(
            dbc.CardBody(id='card', children=[
                ...
                dbc.Row(className='cardBody', children=[
                    cardLayout(html.Div(
                        dcc.Graph(id='jodhpurGasesLinedGraph', className='graphPlot', figure={})))
                ...
                dbc.Row(children=[
                    ...
                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(
                            dcc.Graph(id='jodhpurGasesBoxPlot', className='graphPlot', figure={})))
                    ], width=7),

                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(
                            dcc.Graph(id='jodhpurGasesMonthlyPlot', className='graphPlot', figure={})))
                    ], width=5)
                ]),
                html.Br(), html.Br(),

                dbc.Row(children=[
                    cardLayout(html.Iframe(srcDoc=JodhpurMainElements.html_arima(), style={
                        'height': '500px',
                        'width': '1450px',
                    })),
                    html.Br(), html.Br(),
                ...
                dbc.Row(children=[
                    dbc.Collapse(id='jod_collapse', is_open=False, children=[
                        ...
                        cardLayout(
                            html.Iframe(id="jod_comp_analysis", srcDoc="", style={
                                'height': '500px',
                                'width': '1450px',
                            })
                        ),
                ...
                dbc.Row(children=[
                    cardLayout(html.Iframe(srcDoc=JodhpurMainElements.comparingScenarios(), style={
                        'height': '500px',
                        'width': '1450px',
                    })),
                    html.Br(), html.Br(),
...
@app.callback(
    Output(component_id='jod_comp_analysis', component_property='srcDoc'),
    Input(component_id='slct_metric', component_property='value')
)
def comparitiveAnalysis(value):
    if value == "rmse":
        srcDoc = JodhpurMainElements.comparativeAnalysisRMSE()
    elif value == "mape":
        srcDoc = JodhpurMainElements.comparativeAnalysisMAPE()
    elif value == "mae":
        srcDoc = JodhpurMainElements.comparativeAnalysisMAE()
    elif value == "me":
        srcDoc = JodhpurMainElements.comparativeAnalysisME()
    elif value == "mse":
        srcDoc = JodhpurMainElements.comparativeAnalysisMSE()
    elif value == "mpe":
        srcDoc = JodhpurMainElements.comparativeAnalysisMPE()
    else:
        srcDoc = None
...
@app.callback(
    [Output(component_id='jodhpurGasesLinedGraph', component_property='figure'),
     Output(component_id='jodhpurGasesBoxPlot', component_property='figure'),
     Output(component_id='jodhpurGasesMonthlyPlot',
...
@app.callback(
    Output("jod_collapse", "is_open"),
    [Input("jod_collapse-button", "n_clicks")],
    [State("jod_collapse", "is_open")],
)
def toggle_jod_collapse(n, is_open):
    if n:
        return not is_open
    return is_open
