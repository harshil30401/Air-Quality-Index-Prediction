...
city = pd.read_csv(r"C:\Users\DELL\Desktop\Text Editors & Softwares\Python\Dash\Air-Quality-Index-Prediction\datasets\Kanpur.csv")
...
layout = html.Div(id = 'parent', children = [
    ...
    html.Header(id='header', children=[
        html.Img(src=app.get_asset_url(r"C:\Users\DELL\Desktop\Text Editors & Softwares\Python\Dash\Air-Quality-Index-Prediction\photos\kanpur.jpg"))
    ]),
        ...
        dbc.Card(
            dbc.CardBody(id= 'card', children=[
                ...
                dbc.Row(className='cardBody', children=[
                    cardLayout(html.Div(dcc.Graph(id = 'kanpurGasesLinedGraph', className='graphPlot', figure = {})))
                ], style={'padding':'5px', 'color':'blue'}),

                dbc.Row(children=[

                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(dcc.Graph(id = 'kanpurGasesBoxPlot', className='graphPlot', figure = {})))
                    ], width=7),

                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(dcc.Graph(id = 'kanpurGasesMonthlyPlot', className='graphPlot', figure = {})))
                    ], width=5)
                ]),

                dbc.Row(children=[
                    cardLayout(html.Iframe(srcDoc=KanpurMainElements.comparativeAnalysis(), style={
                            'height':'500px',
                            'width':'1450px',
                        })
                    )
...
@app.callback(
    ...
    Output(component_id='kanpurGasesMonthlyPlot', component_property='figure')
    ],
