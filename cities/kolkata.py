...
layout = html.Div(id='kolkataParent', children=[

    # html.Header(id='header', children=[
    #     html.H1("Kolkata")
    #     # html.Img(id='displayImage',src=app.get_asset_url(f"{rootDirectory}/Air-Quality-Index-Prediction/photos/kolkata.jpg"))
    # ]),
    headerComponent(cityName, "April 2018", math.floor(cityAQI[cityName])),
        ...
        dbc.Card(
            dbc.CardBody(id='card', children=[
                ...
                dbc.Row(className='cardBody', children=[
                    cardLayout(html.Div(
                        dcc.Graph(id='kolkataGasesLinedGraph', className='graphPlot', figure={})))
                ...
                dbc.Row(children=[
                    ...
                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(
                            dcc.Graph(id='kolkataGasesBoxPlot', className='graphPlot', figure={})))
                    ], width=7),

                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(
                            dcc.Graph(id='kolkataGasesMonthlyPlot', className='graphPlot', figure={})))
                    ], width=5)
                ]),
                html.Br(), html.Br(),
                ...
                html.Div(id=f'buttonDiv{cityName}', children=[
                    dbc.Button(
                        ...
                        id=f"kol_collapse-button{cityName}",
                        className="mb-3",
                        color="primary",
                        n_clicks=0,
                ...
                dbc.Row(children=[
                    dbc.Collapse(id=f'kol_collapse{cityName}', is_open=False, children=[
                        ...
                        cardLayout(
                            html.Iframe(id=f"kol_comp_analysis{cityName}", srcDoc="", style={
                                'height': '500px',
                                'width': '1450px',
                            })
                        ),
...
@app.callback(
    Output(component_id=f'kol_comp_analysis{cityName}', component_property='srcDoc'),
    Input(component_id=f'slct_metric{cityName}', component_property='value')
)
def comparitiveAnalysis(value):
    if value == "rmse":
        srcDoc = KolkataMainElements.comparativeAnalysisRMSE()
    elif value == "mape":
        srcDoc = KolkataMainElements.comparativeAnalysisMAPE()
    elif value == "mae":
        srcDoc = KolkataMainElements.comparativeAnalysisMAE()
    elif value == "me":
        srcDoc = KolkataMainElements.comparativeAnalysisME()
    elif value == "mse":
        srcDoc = KolkataMainElements.comparativeAnalysisMSE()
    elif value == "mpe":
        srcDoc = KolkataMainElements.comparativeAnalysisMPE()
    else:
        srcDoc = None
...
@app.callback(
    [Output(component_id='kolkataGasesLinedGraph', component_property='figure'),
     ...
     Output(component_id='kolkataGasesMonthlyPlot',
...
@app.callback(
    Output(f"kol_collapse{cityName}", "is_open"),
    [Input(f"kol_collapse-button{cityName}", "n_clicks")],
    [State(f"kol_collapse{cityName}", "is_open")],
)
def toggle_kol_collapse(n, is_open):
    if n:
        return not is_open
    return is_open
