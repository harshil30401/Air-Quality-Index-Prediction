...
layout = html.Div(id = 'parent', children = [
    ...
    html.Header(id='header', children=[
        html.Img(src=app.get_asset_url(r"C:\Users\DELL\Desktop\Text Editors & Softwares\Python\Dash\Air-Quality-Index-Prediction\photos\mumbai.jpg"))
    ]),
        ...
        dbc.Card(
            dbc.CardBody(id= 'card', children=[
                ...
                dbc.Row(className='cardBody', children=[
                    cardLayout(html.Div(dcc.Graph(id = 'mumbaiGasesLinedGraph', className='graphPlot', figure = {})))
                ], style={'padding':'5px', 'color':'blue'}),

                dbc.Row(children=[

                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(dcc.Graph(id = 'mumbaiGasesBoxPlot', className='graphPlot', figure = {})))
                    ], width=7),

                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(dcc.Graph(id = 'mumbaiGasesMonthlyPlot', className='graphPlot', figure = {})))
                    ], width=5)
                ]),
...
@app.callback(
    ...
    Output(component_id='mumbaiGasesMonthlyPlot', component_property='figure')
    ],
