            ...
            dbc.CardBody(id='card', children=[
                ...
                html.Div(id=f'buttonDiv{cityName}', children=[
                    dbc.Button(
                        ...
                        id=f"pat_collapse-button{cityName}",
                        className="mb-3",
                        color="primary",
                        n_clicks=0,
                ...
                dbc.Row(children=[
                    dbc.Collapse(id=f'pat_collapse{cityName}', is_open=False, children=[
                        ...
                        cardLayout(
                            html.Iframe(id=f"pat_comp_analysis{cityName}", srcDoc="", style={
                                'height': '500px',
                                'width': '1450px',
                            })
                        ),
...
@app.callback(
    Output(component_id=f'pat_comp_analysis{cityName}', component_property='srcDoc'),
...
@app.callback(
    Output(f"pat_collapse{cityName}", "is_open"),
    [Input(f"pat_collapse-button{cityName}", "n_clicks")],
    [State(f"pat_collapse{cityName}", "is_open")],
)
def toggle_pat_collapse(n, is_open):
    if n:
        return not is_open
    return is_open
