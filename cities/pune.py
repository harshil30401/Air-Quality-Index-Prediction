...
from backend.puneBackend import PuneMainElements
...
layout = html.Div(id='puneParent', children=[

    # html.Header(id='header', children=[
    #     html.H1("Pune")
    #     # html.Img(id='displayImage',src=app.get_asset_url(f"{rootDirectory}/Air-Quality-Index-Prediction/photos/pune.jpg"))
    # ]),
        ...
        dbc.Card(
            dbc.CardBody(id='card', children=[
                ...
                dbc.Row(className='cardBody', children=[
                    cardLayout(html.Div(
                        dcc.Graph(id='puneGasesLinedGraph', className='graphPlot', figure={})))
                ...
                dbc.Row(children=[
                    ...
                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(
                            dcc.Graph(id='puneGasesBoxPlot', className='graphPlot', figure={})))
                    ], width=7),

                    dbc.Col(className='cardBody', children=[
                        cardLayout(html.Div(
                            dcc.Graph(id='puneGasesMonthlyPlot', className='graphPlot', figure={})))
                    ], width=5)
                ]),
                html.Br(), html.Br(),

                dbc.Row(children=[
                    cardLayout(html.Iframe(srcDoc=PuneMainElements.html_arima(), style={
                        'height': '500px',
                        'width': '1450px',
                    })),
                    html.Br(), html.Br(),
                ...
                dbc.Row(children=[
                    dbc.Collapse(id='pun_collapse', is_open=False, children=[
                        ...
                        cardLayout(
                            html.Iframe(id="pun_comp_analysis", srcDoc="", style={
                                'height': '500px',
                                'width': '1450px',
                            })
                        ),
                ...
                dbc.Row(children=[
                    cardLayout(html.Iframe(srcDoc=PuneMainElements.comparingScenarios(), style={
                        'height': '500px',
                        'width': '1450px',
                    })),
                    html.Br(), html.Br(),
...
@app.callback(
    Output(component_id='pun_comp_analysis', component_property='srcDoc'),
    Input(component_id='slct_metric', component_property='value')
)
def comparitiveAnalysis(value):
    if value == "rmse":
        srcDoc = PuneMainElements.comparativeAnalysisRMSE()
    elif value == "mape":
        srcDoc = PuneMainElements.comparativeAnalysisMAPE()
    elif value == "mae":
        srcDoc = PuneMainElements.comparativeAnalysisMAE()
    elif value == "me":
        srcDoc = PuneMainElements.comparativeAnalysisME()
    elif value == "mse":
        srcDoc = PuneMainElements.comparativeAnalysisMSE()
    elif value == "mpe":
        srcDoc = PuneMainElements.comparativeAnalysisMPE()
    else:
        srcDoc = None
...
@app.callback(
    [Output(component_id='puneGasesLinedGraph', component_property='figure'),
     Output(component_id='puneGasesBoxPlot', component_property='figure'),
     Output(component_id='puneGasesMonthlyPlot',
...
@app.callback(
    Output("pun_collapse", "is_open"),
    [Input("pun_collapse-button", "n_clicks")],
    [State("pun_collapse", "is_open")],
)
def toggle_pun_collapse(n, is_open):
    if n:
        return not is_open
    return is_open
